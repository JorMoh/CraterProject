...
Assert.assertTrue(item.addItemBtn.isDisplayed());
}
...
public void user_enters_the_details_for_name_price_unit_and_description() {
	...
	item.nameField.sendKeys("lG TV1");
	utils.waitUntilElementVisible(item.priceField);
	item.addedItemPrice.sendKeys("666.44");
	
	utils.waitUntilElementVisible(item.unitField);
...
@Then("The item information entered for the following sections:Name,price,unit and DESCRIPTION. should be saved in the application database.")
public void the_item_information_entered_for_the_following_sections_name_price_unit_and_description_should_be_saved_in_the_application_database() {
